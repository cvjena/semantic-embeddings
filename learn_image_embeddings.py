...
if __name__ == '__main__':
    ...
    arggroup.add_argument('--loss', type = str, default = 'mse', choices = ['mse', 'vmf'], help = 'Loss function for learning embeddings. Use "mse" (mean squared error) for distance-based and "vmf" (von Mises-Fisher) for similarity-based L2-normalized embeddings.')
    ...
    arggroup.add_argument('--vmf_concentration', type = float, default = 1.0, help = 'Concentration parameter of the von Mises-Fisher loss (if used). Larger values enforce tighter distributions.')
    ...
    if args.max_decay > 0:
        ...
        decay = 0.0
    loss = utils.von_mises_fisher_loss(args.vmf_concentration) if args.loss == 'vmf' else utils.squared_distance
    if args.cls_weight > 0:
        par_model.compile(optimizer = keras.optimizers.SGD(lr=args.sgd_lr, decay=decay, momentum=0.9, clipnorm = args.clipgrad),
                          loss = { 'embedding' : loss, 'prob' : 'categorical_crossentropy' },
                          loss_weights = { 'embedding' : 1.0, 'prob' : args.cls_weight },
                          metrics = { 'embedding' : utils.nn_accuracy(embedding, dot_prod_sim = (args.loss == 'vmf')), 'prob' : 'accuracy' })
    else:
        par_model.compile(optimizer = keras.optimizers.SGD(lr=args.sgd_lr, decay=decay, momentum=0.9, clipnorm = args.clipgrad),
                          loss = loss,
                          metrics = [utils.nn_accuracy(embedding, dot_prod_sim = (args.loss == 'vmf'))])
