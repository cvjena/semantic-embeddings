...
def build_ensemble(architecture, num_classes, embeddings, losses):
    ...
    for i, loss in enumerate(losses):
        if loss == 'inv_corr':
            embed_networks[i] = keras.models.Model(embed_networks[i].inputs, keras.layers.Lambda(utils.l2norm, name = 'l2norm')(embed_networks[i].output), name = 'cnn{}'.format(i+1))
...
def transform_inputs(X, y, embeddings):
    
    return X, [keras.utils.to_categorical(y, embeddings[0].shape[0]), keras.utils.to_categorical(y, embeddings[0].shape[0])] + [embed[y] for embed in embeddings]
...
if __name__ == '__main__':
    ...
    if args.gpus <= 1:
        model = build_ensemble(args.architecture, data_generator.num_classes, embeddings, losses)
        par_model = model
    else:
    ...
    par_model.compile(optimizer = keras.optimizers.SGD(lr=args.sgd_lr, decay=decay, momentum=0.9, clipnorm = args.clipgrad),
                      loss = dict([('prob', 'categorical_crossentropy'), ('cnn0', 'categorical_crossentropy')] + [('cnn{}'.format(i), utils.inv_correlation if loss == 'inv_corr' else utils.squared_distance) for i, loss in enumerate(losses, 1)]),
                      loss_weights = dict([('prob', args.cls_weight)] + [('cnn{}'.format(i), 1.0) for i in range(len(embeddings) + 1)]),
                      metrics = dict([('prob', 'accuracy'), ('cnn0', 'accuracy')] + [('cnn{}'.format(i), utils.nn_accuracy(embedding, dot_prod_sim = (loss == 'inv_corr'))) for i, (embedding, loss) in enumerate(zip(embeddings, losses), 1)])
    )

    batch_transform_kwargs = { 'embeddings' : embeddings }
